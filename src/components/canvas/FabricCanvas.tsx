...
export const FabricCanvas = forwardRef<FabricCanvasHandle, FabricCanvasProps>(
    ({ width = 800, height = 600, className = "" }, ref) => {
        ...
        useEffect(() => {
            ...
            // handles inline text editing (2way editing)
            ...
            canvas.on("editing:exited" as any, (e) => {
                const target = e.target as fabric.Textbox;
                const layerId = (target as any).data?.layerId;
                if (layerId && target.text) {
        ...
        // update canvas size on img dimension change
        useEffect(() => {
            const canvas = fabricCanvasRef.current;
        ...
        // Sync text layers with Fabric objects
        useEffect(() => {
            const canvas = fabricCanvasRef.current;
            if (!canvas) return;
            ...
            // remove objs that are not in textLayers
